...
%Huvudfunktion%Kräver följande .m filer: hooke, plani4e, solveq, extract, designDomain, plani4s, eldraw2
XVAL = projektFHLN01;
...
kraft = -50; %Kraft (N)
xval = 0.75 * ones(nelm, 1); %Designvariabler
r_min_compli = 3 * le;
xforce_left = 2 * le;
...
%Spänningsvillkor
if m == 2
[g_stress, dg_stress, vm] = adjoint_stress(edof, Ex, Ey, kelem, K, ac, bc, D, xval_projected, p, sigma_allow, de, M, nelm);
...
meshplot(edof,vm, Ex, Ey, Ed, k, kt, kk, kkk, kkkk, kkkkk, coord, dof, maxIter, f0_history, xmat)

%Känslighetsanalys
elementToPerturb = 89; %Element som undersöks
compareStressSens(elementToPerturb, xval, Lx, Ly, le, de, p_stress, sigma_allow, q_stress, bc_xmin, bc_xmax, xforce_left, kraft, M, beta, eta, edof, Ex, Ey, kelem, ac, bc, D, nelm,p);
...
function [g_stress, dg_stress, vm] = adjoint_stress(edof, Ex, Ey, kelem, K, ac, bc, D, xval_projected, p, sigma_allow, de, M, nelm)
ndof = size(K,1);
vm = zeros(nelm,1);
P = [2 -1 0; -1 2 0; 0 0 6];
...
%Känsligheter
function [g_stress, dg_stress] = evaluateStressConstraint(xval_in, Lx, Ly, le, de, p_stress, sigma_allow, M, beta, eta,p)
[coord, dof, enod, edof, Ex, Ey] = designDomain(Lx, Ly, le);
nelm = (Lx/le)*(Ly/le);
...
function compareStressSens(elementToPerturb, xval_baseline, Lx, Ly, le, de, p_stress, sigma_allow, q_stress, bc_xmin, bc_xmax, xforce_left, kraft, M, beta, eta, edof, Ex, Ey, kelem, ac, bc, D, nelm,p)
...
[g_stress_base, dg_stress_base] = evaluateStressConstraint(xval_baseline, Lx, Ly, le, de, p_stress, sigma_allow, M, beta, eta,p);
[g_stress_pert, ~] = evaluateStressConstraint(xval_perturbed, Lx, Ly, le, de, p_stress, sigma_allow, M, beta, eta,p);
...
%Plottar resultat
function meshplot(edof,vm, Ex, Ey, Ed, k, kt, kk, kkk, kkkk, kkkkk, coord, dof, maxIter, f0_history, xmat)
if k==1
figure();
patch(Ex', Ey', 1);
plotpar=[1 2 0];
...
if kkk==1
figure()
xmat_rotated = rot90(xmat, -3); % Rotate counterclockwise
imagesc(xmat_rotated);
colormap(flipud(gray)); % Black for 1, White for 0
colorbar;
clim([0 1]); % Ensure correct color scaling
axis equal;
